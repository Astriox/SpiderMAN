...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("🙏 SORRY DUDE YOUR CLICKED MY OLD BUTTON PLEASE SEARCH AGAIN .", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("gfile"):
        ...
        buttons = [[
            InlineKeyboardButton('🤖 𝚂𝚄𝙿𝙿𝙾𝚁𝚃', url='https://t.me/mkn_bots_updates'),
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.ABOUT_TXT.format(bot_name = temp.B_NAME),
            reply_markup=reply_markup,
            parse_mode='html'
        )
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⏹️ 𝙱𝚄𝚃𝚃𝙾𝙽 𝙵𝙾𝚁𝙼𝙰𝚃', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
